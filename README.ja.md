...
Agentには前方減速と横回避を、Obstacleには接線ステアリングを適用します。
...
正面に別のAgentがいる場合に加える横方向速度の比率です。Agent同士は進行方向に対して同じ通行側を
選びます。Obstacle回避では、この横方向速度の加算ではなく接線ステアリングを使用します。

- `0`でAgent間の追加の横回避を無効化し、主に減速する
...
Agentが`CollisionPredictionTime`以内にObstacleへ接近すると、希望速度を維持したまま、拡張した
Obstacleの接線方向へ希望方向を連続的に回転させます。Circleは中心、Segmentは線分上の最近接点を
基準とするため、Segmentは拡張Capsuleとして振る舞います。拡張量にはAgent半径の半分の幾何的余裕と、
速度応答の1時定数ぶんの移動距離（予測時間内の移動距離が上限）を加えます。選択した通過側は予測時間に
短い余白を加えた期間保持し、回避方向の反転を抑えます。予測が間に合わない場合も、移動後の非貫通制約が
AgentをObstacle表面へ戻します。
