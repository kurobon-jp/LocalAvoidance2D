...
Agentが`CollisionPredictionTime`以内にCircle Obstacleへ接近すると、希望速度を維持したまま、
拡張円の接線方向へ希望方向を連続的に回転させます。拡張円にはAgent半径の半分の幾何的余裕と、
速度応答の1時定数ぶんの移動距離（予測時間内の移動距離が上限）を加えます。Segment Obstacleは
従来どおり接触前の減速と横回避を行い、近い端点側を優先します。選択した通過側は予測時間に短い
余白を加えた期間保持し、回避方向の反転を抑えます。予測が間に合わない場合も、移動後の非貫通制約が
